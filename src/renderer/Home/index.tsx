...
const Home = ()=>{
  ...
  const [directoryPath, setDirectoryPath] = useState('C:\\Program Files\\Zoaverse Business Hub');
  ...
  const handleImagePrev=()=>{
    ...
    return (
        <div className="!z-10 !h-[631px] !w-full font-montserrat !relative" style={{
  ...
  filter: 'contrast(0.9)'
        ...
        <div className="h-full relative z-20 flex flex-row ">
          ...
          Zoaverse provides a wide selection of customizable venues that makes it easy to get started in the metaverse with just a few clicks.
          </div>
  ...
  <img src={footerImg} alt="" className="w-full h-auto rounded-bl-[30px]" style={{ borderBottomLeftRadius: '30px' }}/>
  <div className="absolute inset-0 flex flex-row items-center justify-start gap-10 px-12">
    <button className="px-8 py-3 bg-skyBlue rounded-full relative top-4 text-white hover:bg-blue-600 "><span className="flex flex-row justify-between items-center gap-6 w-full"> <span className="text-sm text-lightWhite">Begin Install</span> <span><img src={goIcon} alt="" /></span></span> </button>
    <div className="round relative top-4 flex flex-row items-center justify-center gap-8">
    <input type="checkbox" checked={checkbox} id="checkbox" onClick={()=>setCheckBox(!checkbox)} className=""/>
...
Auto update the platform whenever a new update is available 
            ...
            <div className="flex flex-row items-center gap-3">
              <div className={`p-2 bg-white bg-red-warm hover:bg-gray-200  transition-colors duration-200 ease-in-out transform  active:scale-95 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50 h-12 w-12 rounded-full flex items-center justify-center ${index === 0 ? 'bg-opacity-60 cursor-not-allowed hover:bg-opacity-80 hover:scale-100':'cursor-pointer hover:scale-105'}`} onClick={()=>handleImagePrev()}>
                <img src={arrowIcon} alt="" className="self-center -rotate-45 "  style={{rotate:'180deg'}} />

              </div>
              <div className={`p-2 bg-white bg-red-warm hover:bg-gray-200  transition-colors duration-200 ease-in-out transform hover:scale-105 active:scale-95 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50 h-12 w-12 rounded-full flex items-center justify-center ${index === 3 ? 'bg-opacity-60 cursor-not-allowed hover:bg-opacity-60':'cursor-pointer'}`} onClick={()=>handleImageNext()}>
                <img src={arrowIcon} alt="" className="self-center -rotate-45 " />
              </div>
            </div>
